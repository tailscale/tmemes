...
- The server is `tmemes`, a standalone Go binary using `tsnet`. Run
  ...
  ```
  TS_AUTHKEY=$KEY go run ./cmd/tmemes
  ```
...
- The server "database" is a directory of files. Use `--store` to set the
...
- UI elements are generated by Go HTML templates in `server/ui`. These are
  statically embedded into the importable `server` package and served by its
  handlers.

- Static assets needed by the UI are stored in `server/static`. These are
